...
function extractTimelinePoint(snapshotMeta, game, side) {
  ...
  return {
    ...
    total_over_price: game.market?.total?.over_price ?? null,
    total_under_price: game.market?.total?.under_price ?? null,
    run_line_points: game.market?.run_line?.[`${side}_points`] ?? null,
    run_line_price: game.market?.run_line?.[`${side}_price`] ?? null,
  ...
  for (const snapshot of snapshots) {
    for (const game of snapshot.payload.games || []) {
      ...
      target.snapshots.push({
        ...
        total_over_price: game.market?.total?.over_price ?? null,
        total_under_price: game.market?.total?.under_price ?? null,
        run_line: game.market?.run_line || null,
